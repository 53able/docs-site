...
このファイルは `SKILL.md` Phase 3-4 でコンポーネント選択に迷った時だけ読む（JiT ロード）。
...
既存ページとの重複を避けるため、生成前にチェックする。

### 既存ページのフォント使用状況（2025-03 時点）

| ページ | フォント |
|--------|---------|
| react-grab, defuddle, opentui 等 多数 | DM Sans + Fira Code |
| skills-best-practices | IBM Plex Sans + IBM Plex Mono |
