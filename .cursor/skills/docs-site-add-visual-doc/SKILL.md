...
| `docs/opentui.html` または `docs/react-grab.html` のいずれか 1 件 | 解説ページ 1 本の体裁：**Unfurl**（`<head>` の description ＋ Open Graph ＋ Twitter Card のメタブロック）、THEME（:root + dark）、Spell UI static（.ve-card, .ve-card--elevated, .ve-card--recessed, .ve-card--hero, .ve-section-label, .ve-code-block, .ve-table-wrap, .ve-kpi-card, .ve-callout）、`.wrap` / `.toc` / `.main`、`.sec-head`、Scroll Spy 用の `<script>` |
